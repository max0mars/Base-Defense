# Enemy List

## Basic
- **Health**: 100
- **Damage**: 10
- **Speed**: 25
- **Spawn Cost**: 10
- **Spawn Weight**: 60
- **Upgrades**:
  - Blast Shields: Take 30% less explosive damage.
  - Mitosis: 10% chance to split into 2 Basic enemies on death.

## Speeder
- **Health**: 25
- **Damage**: 10
- **Speed**: 110
- **Spawn Cost**: 25
- **Spawn Weight**: 45
- **Upgrades**:
  - Overdrive: Speed +20%
  - Hardened Shell: HP +30%
  - Anti-Grav Plating: Speeders can now fly over blockers.

## Tank
- **Health**: 1000
- **Damage**: 30
- **Speed**: 15
- **Spawn Cost**: 55
- **Spawn Weight**: 25
- **Upgrades**:
  - Behemoth Plating: HP +50%
  - Turbo Engines: Speed +50%

## Flyer
- **Health**: 800
- **Damage**: 30
- **Speed**: 18
- **Spawn Cost**: 50
- **Spawn Weight**: 25
- **Upgrades**:
  - Swift Swarm: Speed +40%
  - Precision Wings: HP +40%

## Carrier
- **Health**: 300
- **Damage**: 20
- **Speed**: 18
- **Spawn Cost**: 45
- **Spawn Weight**: 30
- **Upgrades**:
  - Rapid Deployment: Spawn Rate +30%
  - Swarm Brood: Carrier spawns +1 Speeder.

## Armored
- **Health**: 300
- **Damage**: 15
- **Speed**: 20
- **Spawn Cost**: 40
- **Spawn Weight**: 30
- **Upgrades**:
  - Dreadnought Plating: HP +40%
  - Even Stronger Armor: Normal resistance +20%
  - Energy Shielding: Energy resistance +25%

## Guardian
- **Health**: 250
- **Damage**: 20
- **Speed**: 22
- **Spawn Cost**: 75
- **Spawn Weight**: 25
- **Upgrades**:
  - Guardian Aura: Guardian projects a 25% damage reduction aura to nearby allies.
  - Shield Overcharge: Doubles the amount of shields granted to allies.

## Duplicator
- **Health**: 200
- **Damage**: 20
- **Speed**: 22
- **Spawn Cost**: 40
- **Spawn Weight**: 40
- **Upgrades**:
  - Symbiotic Burst: On duplication, releases a healing wave affecting all enemies.
  - Accelerated Mitosis: Each duplication stage has increased movement speed.
  - Hyper-Replication: The last duplication stage spawns 1 extra clone.
